...
class thr_zisco extends MY_App {
	...
	{
		$this->template->set('pagetitle','Form Daftar THR Zisco');
		...
		$this->template->load('default','fthr_zisco/filterForm',$data);
	}

	public function thr_list(){
		$this->config->set_item('mySubMenu', 'mn42');
		$this->template->set('pagetitle','Daftar THR Zisco');		
		$tahun=$this->input->post('tahun');	
		$isXls=$this->input->post('isXls');	
		
		//$strList = "SELECT p.*,  period_diff( date_format( now( ) , '%Y%m' ) , date_format( tgl_aktif, '%Y%m' ) ) SELISIH from pegawai p where  status_aktif=1 ".($id_cab==""?"":" and id_cabang=$id_cab  ")." and p.id_jab in (35,36) order by id_cabang";
		$strList = "SELECT p.*,  period_diff( date_format( now( ) , '%Y%m' ) , date_format( TGL_AWAL_KONTRAK, '%Y%m' ) ) SELISIH from pegawai p where  status_aktif=1  and p.id_jab in (35,36) order by id_cabang";

		$strCek="select count(*) CEK, VALIDASI, ID from thr_validasi where tahun='$tahun'  and jenis='zisco_transport'";
		//get tunjangan anak
		...
		if ($rsCek->CEK<=0){
			$title="Daftar Data THR Zisco (NEW)";
			$sts="new";
			$view="thr_zisco_list";
		}else{	
			$data['sts_validasi'] = $rsCek->VALIDASI;
			$data['id_validasi']=$rsCek->ID;
			$strList="select t.*, p.NIK, p.NAMA, p.TGL_AKTIF, p.TGL_AWAL_KONTRAK,p.STATUS_PEGAWAI, p.ID_JAB, p.ID_CABANG from thr_zisco t, pegawai p where p.nik=t.nik   and tahun='".$tahun."' and id_validasi=".$rsCek->ID;
			//$data['row'] = $this->db->query()->result();	
			$data['row'] = $this->db->query($strList)->result();	
			if (date('Y')>=$tahun){	
				//boleh edit, ambil data tabel thr
				$title="Daftar Data THR Zisco (OPEN)";
				$sts="edit";					
			}else{
				$title="Daftar Data THR Zisco (CLOSED)";
				$sts="disabled";
				
			}
			$view="thr_zisco_listed";
		}
		$data['strList'] = $strList;
		$data['sts']=$sts;
		if ($isXls==0){
			$this->template->set('pagetitle',$title." ".$tahun);	
			$this->template->load('default','fthr_zisco/'.$view,$data);
		}else{
			//$objPHPExcel = new PHPExcel();
			$html=$this->load->view('fthr_zisco/thr_zisco_excel' , $data, true);
			...
			$objWriter->save('assets/excel/thrzisco_'.$tahun.'.xlsx');		
			
			$data['isi']='assets/excel/thrzisco_'.$tahun.'.xlsx';
	...
	public function save_thr_zisco(){
		if($this->input->post()) {
			...
			if ($this->form_validation->run() == TRUE){
				...
				try {
					...
					if (trim($sts)=="new"){
						...
						$this->db->query("delete from thr_zisco where id_validasi=".$id_validasi);		
						$this->db->trans_commit();
					}
					...
					for($r=1;$r<=$this->input->post('jmlRow');$r++){
						if ($this->input->post('flag_'.$r)=="1"){
							...
							$data = array(									
									...
									'UANG_TRANSPORT' => $this->input->post('komp_'.$r.'_0'),
									'TUNJ_JABATAN' => $this->input->post('komp_'.$r.'_1'),									
									'PENYESUAIAN' => $this->input->post('komp_'.$r.'_2'),
									'KOREKSI' => $this->input->post('komp_'.$r.'_3'),
									'POTONGAN_DANSOS' => $this->input->post('komp_'.$r.'_4'),
									'POTONGAN_LAIN' => $this->input->post('komp_'.$r.'_5'),
									'TOTAL' => $this->input->post('subGrandTotal_'.$r)
								);
								if ($this->db->insert('thr_zisco', $data)){									
									$this->db->trans_commit();
								} else {
									throw new Exception("gagal simpan");
								}
	...
	public function cetak_slip(){
		...
		$html.="</td>";
		$html.="</tr>";
		$html.="<tr><td colspan=2 style='text-align:center'><h2>SLIP THR ZISCO YATIM MANDIRI<br>TAHUN ".$thn."</h2></td></tr>";
		$html.="<tr><tD colspan=2></td></tr>";
		$html.="<tr><tD >NAMA </td><tD> : ".$rsOut->NAMA."</td></tr>";
		$html.="<tr><tD >JABATAN</td><tD> : ".$rsMst->NAMA_JAB."</td></tr>";
		$html.="</table>";
		$tot_pendapatan=$rsOut->uang_transport + $rsOut->tunj_jabatan + $rsOut->penyesuaian+$rsOut->total;
		$html.="<table class=\"mydata\" >";
		$html.="<tr><tD colspan=4><u>A. PENDAPATAN</u></td></tr>";
		$html.="<tr><td>1</td><td>Uang Transport</td><td>:</td><td style='text-align:right'>Rp. ".number_format($rsOut->uang_transport,0,',','.')."</td></tr>";
		$html.="<tr><td>2</td><td>Tunjangan Jabatan</td><td>:</td><td style='text-align:right'>Rp. ".number_format($rsOut->tunj_jabatan,0,',','.')."</td></tr>";
		$html.="<tr><td>3</td><td>Penyesuaian</td><td>:</td><td style='text-align:right'>Rp. ".number_format($rsOut->penyesuaian,0,',','.')."</td ></tr>";
		$html.="<tr><td>4</td><td>Koreksi</td><td>:</td><td style='text-align:right'>Rp. ".number_format($rsOut->koreksi,0,',','.')."</td></tr>";
		$html.="<tr><th COLSPAN=3><b>Total Pendapatan</b></th><th style='text-align:right'><b>Rp. ".number_format($tot_pendapatan,0,',','.')."</b></th></tr>";
		$tot_pot=$rsOut->potongan_dansos + $rsOut->potongan_lain ;
		$html.="<tr><tD colspan=4><u>B. POTONGAN</u></td></tr>";
		$html.="<tr><td>1</td><td>Dansos</td><td>:</td><td style='text-align:right'>Rp. ".number_format($rsOut->potongan_dansos,0,',','.')."</td></tr>";
		$html.="<tr><td>2</td><td>THT</td><td>:</td><td style='text-align:right'>Rp. ".number_format($rsOut->potongan_lain,0,',','.')."</td></tr>";
		...
		$path=$this->createPath('slip_thr','zisco_transport', '', $thn, date('m') );	//csv path hanya s.d thn masuk nama file
		$fileName="SLIP_THR_Zisco_".$thn."-".$nik.".pdf";		
	...
	function slipLoop(){
		...
		$str = "select * from thr_zisco where TAHUN='$thn' and id_validasi=".$id_validasi." ORDER BY `NIK` ";
	...
	public function loop_cetak_slip($param){
		...
		$html.="<tr><tD >JABATAN</td><tD> : ".$rsMst->NAMA_JAB."</td></tr>";
		$html.="</table>";
		$totpend= $rsOut->uang_transport + $rsOut->tunj_jabatan + $rsOut->penyesuaian + $rsOut->koreksi ;
		$html.="<table class=\"mydata\" >";
		$html.="<tr><tD colspan=4><u>A. PENDAPATAN</u></td></tr>";
		$html.="<tr><td>1</td><td>Uang Transport</td><td>:</td><td style='text-align:right'>Rp. ".number_format($rsOut->uang_transport,0,',','.')."</td></tr>";
		$html.="<tr><td>2</td><td>Tunjangan Jabatan</td><td>:</td><td style='text-align:right'>Rp. ".number_format($rsOut->tunj_jabatan,0,',','.')."</td></tr>";
		$html.="<tr><td>3</td><td>Penyesuaian</td><td>:</td><td style='text-align:right'>Rp. ".number_format($rsOut->penyesuaian,0,',','.')."</td ></tr>";
		$html.="<tr><td>4</td><td>Koreksi</td><td>:</td><td style='text-align:right'>Rp. ".number_format($rsOut->koreksi,0,',','.')."</td></tr>";
		$html.="<tr><th COLSPAN=3><b>Total Pendapatan</b></th><th style='text-align:right'><b>Rp. ".number_format($totpend,0,',','.')."</b></th></tr>";
		$totpot= $rsOut->potongan_dansos + $rsOut->potongan_lain ;
		$html.="<tr><tD colspan=4><u>B. POTONGAN</u></td></tr>";
		$html.="<tr><td>1</td><td>Dansos</td><td>:</td><td style='text-align:right'>Rp. ".number_format($rsOut->potongan_dansos,0,',','.')."</td></tr>";
		$html.="<tr><td>2</td><td>Potongan Lain</td><td>:</td><td style='text-align:right'>Rp. ".number_format($rsOut->potongan_lain,0,',','.')."</td></tr>";
		$html.="<tr><th COLSPAN=3><b>Total Potongan</b></th><th style='text-align:right'><b>Rp. ".number_format($totpot,0,',','.')."</b></th></tr>";
		$html.="<tr><th COLSPAN=3><b>Total Diterima</b></th><th style='text-align:right'><b>Rp. ".number_format(( $totpend+$totpot ),0,',','.')."</b></th></tr>";
		$html.="<tr><tD colspan=4></td></tr>";
		$html.="<tr><td COLSPAN=4 >MASA KERJA : $masaKerja </td></tr>";		
		$html.="<tr><tD colspan=4 style='text-align:right'><i><b>HRD - Yatim Mandiri</b></i></td></tr>";
		$html.="<tr><tD colspan=4 style='text-align:center'>Jika terdapat kesalahan dalam penghitungan, dipersilahkan untuk<br>konfirmasi ke bagian HRD.Kekeliruan penghitungan akan dilakukan<br>koreksi bulan depan.</td></tr>";
