<?php echo form_open('gaji_zisco_bonus/save_gaji_zisco_transport',array('class'=>'form-horizontal','id'=>'myform'));?>
 <div class="row">
	<div class="col-xs-12">
...
<?	//echo "<tr align=center><td colspan=\"".$colspanAll."\">$str</td></tr>";
	if (sizeof($row)==0){	
		...
		//GET ABSEN VALUE
		$jml_tidak_hadir=$hasil->KETIDAKHADIRAN;
...
<td><?=form_input(array('name'=>'subGrandTotal_'.$i,'id'=>'subGrandTotal_'.$i,'class'=>'myform-control','size'=>10,'readonly'=>true, 'value'=>round($sumPer_row+$sumPotPer_row,0) ) );?></td>
	<!-- GENERATE/DOWNLOAD -->
		...
		?>
		<a href="javascript:void(0)" data-url="<?=base_url('gaji_zisco_bonus/cetak_slip/')?>" data-id="<?=$param?>" onclick="singleSlip(this)"><i class="fa fa-edit" title="Generate Slip to .pdf File">Generate&nbsp;pdf</i></a>
		<?}
				echo "</td>";
			}else{
				echo "<td>&nbsp; link</td>";
...
$('#btsimpankel').click(function(){		
		...
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url('gaji_zisco_bonus/save_gaji_zisco_transport');?>',
			data: form_data,				
			dataType: 'json',
			success: function(msg) {
