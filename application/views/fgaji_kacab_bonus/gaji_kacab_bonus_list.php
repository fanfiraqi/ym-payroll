...
$str_pengambilan="SELECT  
IFNULL(SUM(dd.nominal),0) target, IFNULL(SUM(dd.paid),0) realisasi, IFNULL(SUM(dd.diff),0) selisih 
...
d.cabang_id=".$hasil->ID_CABANG."  and d.`type`='RUTIN' and don.registered_at not like '".$thn.'-'.$bln."%' 
AND   d.donatur_id in (select id from donatur where cabang_id=".$hasil->ID_CABANG.")
*/	
$str_pengambilan="select 
...
AND  (case  WHEN d.paid_at is null then  CONCAT(d.year, d.month) ELSE date_format(d.paid_at,'%Y%c') end) ='".$thn.intval($bln)."' 
and d.status in ('DRAFT','PAID', 'FAILED')
and d.cabang_id=".$hasil->ID_CABANG."  and d.`type`='ROUTINE' 
AND   d.donatur_id in (select id from donatur where cabang_id=".$hasil->ID_CABANG.")
) tgt,
...
(SELECT IFNULL(SUM(dnsd.paid),0) 
	...
	WHERE dns.id=dnsd.donasi_id 
	and dns.bkm_id in (select id from bkm where status='VALIDATED') 
	and d.cabang_id=".$hasil->ID_CABANG." AND dns.id=d.id
 	and (case  WHEN dns.paid_at is null then  CONCAT(dns.year, dns.month) ELSE date_format(dns.paid_at,'%Y%c') end) ='".$thn.intval($bln)."' 
	and dns.`type`='ROUTINE'
	) realisasi, 
...
and d.cabang_id=".$hasil->ID_CABANG."  and d.`type`='ROUTINE' AND   d.donatur_id in (select id from donatur where cabang_id=".$hasil->ID_CABANG.")
  ...
  WHERE d.id = dd.donasi_id 
   ...
   d.cabang_id=".$hasil->ID_CABANG."  
     and d.`type`='ROUTINE') pgb_faild";
...
FROM donasi d, detail_donasi dd	
WHERE d.id=dd.donasi_id and d.status in ('PAID', 'FAILED')
AND (case  WHEN d.paid_at is null then  CONCAT(d.year, d.month) ELSE date_format(d.paid_at,'%Y%c') end) ='".$thn.intval($bln)."'  AND d.cabang_id=".$hasil->ID_CABANG."
AND dd.jenis_donasi_id IN (SELECT id FROM jenis_donasi WHERE   payroll_column <>'wakaf') 
and d.`type`='INSIDENTAL' and d.id in (select id from donasi where bkm_id in (select id from bkm where zisco_id =".$hasil->ID."  and status='VALIDATED'))";
...
AND (case  WHEN d.paid_at is null then  CONCAT(d.year, d.month) ELSE date_format(d.paid_at,'%Y%c') end) ='".$thn.intval($bln)."'  AND d.cabang_id=".$hasil->ID_CABANG."
AND dd.jenis_donasi_id IN (SELECT id FROM jenis_donasi WHERE   payroll_column ='wakaf') 
and d.id in (select id from donasi where bkm_id in (select id from bkm where zisco_id =".$hasil->ID." and status='VALIDATED'))";
...
AND d.cabang_id=".$hasil->ID_CABANG." and  d.`type`='ROUTINE'
and d.id in (select id from donasi where bkm_id in (select id from bkm where zisco_id =".$hasil->ID." and status='VALIDATED')
and d.donatur_id in (select id from donatur where registered_at like '".$thn.'-'.$bln."%')
)";

$sql_rutin=$this->donasi_db->query($str_kacab_rutin);
	if ($sql_rutin->num_rows>0){
		$rs_rutin=$sql_rutin->row();
		//$jmlkacab_rutin=($rs_rutin->realisasi > $rs_rutin->target?$rs_rutin->target: $rs_rutin->realisasi);
		$jmlkacab_rutin=$rs_rutin->realisasi;
	}
