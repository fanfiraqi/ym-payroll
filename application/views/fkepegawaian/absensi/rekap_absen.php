...
if (sizeof($row)==0){		
	...
	foreach($row as $hasil){
		...
		$sIjin="select sum(JML_HARI) ALPA_IJIN from ijin_khusus_alpa where (TGL_AWAL BETWEEN '$start_date' AND '$end_date') and (TGL_AKHIR BETWEEN '$start_date' AND '$end_date') and nik='".$hasil->NIK."'";
			$alpaijin=0;
		...
		$hrTerlambat=0;
		//$hrTerlambat=(floor(($hasil->J_TERLAMBAT)/60)>=7?$jml_lembur/7:0);

		//$alpa=$jmlHariKerja-($hasil->J_MASUK+$jml_cuti);
		$alpa=$jmlHariKerja-($jmlHariKerja+$jml_cuti);
		$alpa=($alpa<0?0:$alpa);
	...
	<td><?=form_input(array('name'=>'jml_masuk_'.$i,'id'=>'jml_masuk_'.$i,'class'=>'myform-control','size'=>10, 'value'=>($jmlHariKerja-$jml_cuti), 'onchange'=>"setAlpa(this, '$i')"));?></td>
