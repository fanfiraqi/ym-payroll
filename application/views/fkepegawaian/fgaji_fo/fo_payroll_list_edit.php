...
if (sizeof($row)==0){		
	...
	//echo "<tr align=center><td colspan=20>$jmlbln#".$hasil->TERMIN."#".$hasil->TGL_MULAI."#".date("Y-m-d")."</td></tr>";	
	...
	if ($hasil->TGL_MULAI<=date("Y-m-d")){
		...
		if (sizeof($qPinjaman)>=1){
			...
			if ($qPinjaman->JML_BAYAR<=0 && $thn.$digitBln>=$qPinjaman->thnblnhrsbayar){				
		...
		//TAAWUN & PREMI JHT
			...
			if ($hasil->PROGRAM_JHT==1){				
				...
				$jht_ke=($umurKerja>=37?$hasil->JHT_KE:0);	//edit -> current val
...
<?	//bonus 100-80, blm cek akhir termin
	...
	if ($blnKe>=$hasil->TERMIN ){
		...
		$cekFO=$rsSum->PRLH."#".($target_P*$hasil->TERMIN);
