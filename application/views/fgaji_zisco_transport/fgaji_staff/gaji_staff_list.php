...
<?	//echo "<tr align=center><td colspan=\"".$colspanAll."\">$str</td></tr>";
	if (sizeof($row)==0){	
		...
		//hari kerja ambil dari database
			...
			$PC_mnt=0;
			$jam_lembur=0;
			$skerja="select * from rekap_absensi where periode='".$thn_pre.$bln_pre."' and nik='".$hasil->NIK."'";
		...
		switch ($rowmaster->ID){
			...
			//W7-((H7+I7+(J7*2)+M7)*(W7/25))-(K7*5000)-(L7*10000)
				...
				if (sizeof($rsmakan)>0){
					...
					$nominal = $acuan - ( ($jml_sakit + $jml_ijin + (2 * $jml_ijin) + $T10_3) * ($acuan/25) ) - ( $T10_1*5000 ) - ( $T10_2 * 10000 );
				}
				$cekmasuk="2";
				break;
			...
			case "8":	//Tj. Hari Tua (THT), table
				...
				$str4="select ifnull(nominal,0) nominal from mst_tht where kelompok_jab='".$kelompok_gaji."' and pendidikan='".$hasil->PENDIDIKAN."' ORDER BY id DESC LIMIT 1";
		...
		//$j++; => bertambah 1 utk komp
			...
			case "15":	//Dana Sosial
				...
				if (sizeof($rsdansos)>0){
					...
					$nominal = ($sumPer_row <=$dansos ?$sumPer_row*0.025:( $sumPer_row>=$dansos && $kelompok_gaji=='S' ? $sumPer_row*0.01: ( $sumPer_row>=$dansos && $kelompok_gaji=='M' ? $sumPer_row*0.02 : ( $sumPer_row>=$dansos && $kelompok_gaji=='GM' ? $sumPer_row*0.025 : "CEK LAGI") ) ) );	
					//$nominal=round($nominal, 0);
				}
				$cekmasuk="1";
				break;
