...
?>
	<tr>
		...
		<td><?=str_replace(" ","&nbsp;", strftime("%d %B %Y",strtotime($hasil->TGL_AKTIF)))?><input type="hidden" name="id_cabang_<?=$i?>" id="id_cabang_<?=$i?>" value="<?=$hasil->ID_CABANG?>"></td>
...
$('#btXls').click(function() {		 
		...
		if (pilih==true) {
			$.ajax({
				url: "<?php echo base_url('thr_staff/thr_list'); ?>",
				dataType: 'json',
				type: 'POST',
				data: "tahun="+$("#tahun").val()+"&laz_tasharuf="+$("#laz_tasharuf").val()+"&wilayah="+$("#wilayah").val()+"&isXls=1",				
				success: function(data,  textStatus, jqXHR){					
					//alert("File csv sudah tersimpan");
...
$('#btvalidasi').click(function(){	
	$.ajax({
			...
			success: function(msg) {
				 console.log(msg);
				if(msg.status =='success'){
					$().showMessage('Data  THR Staf berhasil divalidasi.','success',1000);
					setInterval(window.location.reload(), 3000);
				} else {
					$().showMessage('Terjadi kesalahan. Data gagal disimpan.','danger',700);
