...
<div class="row">
	<div class="col-md-6">
		<div class="form-group"><label  class="col-sm-4 control-label">TAHUN</label>
			<div class="col-sm-5"><?=form_input(array('name'=>'tahun','id'=>'tahun','class'=>'form-control','readonly'=>"true", 'value'=>$op_tahun));?></div>
		</div>
	</div>
</div>
<div class="row">
	<div class="col-md-6">
		<div class="form-group"><label class="col-sm-4 control-label">KOTA CABANG</label>
			<div class="col-sm-8"><?=form_dropdown('cabang',$cabang,$id_cabang,'id="cabang" class="form-control" "readonly"=>"true"');?></div>
		</div>
	</div>
</div>
<div class="row">
	<div class="col-md-6">
		<div class="form-group"><label  class="col-sm-4 control-label">DIVISI</label>
			<div class="col-sm-8"><?=form_dropdown('divisi',$divisi,$id_divisi,'id="divisi" class="form-control"');?></div>
		</div>
	</div>
</div>
<div class="row">
...
<?	$i=1;	
	foreach($komponen as $row){
		//checked op
		$res=$this->db->query("select count(*) CEK from mst_thr_staff where TAHUN='$op_tahun' and ID_CABANG=".$id_cabang." AND ID_DIVISI=".$id_divisi." AND ID_KOMPONEN=".$row->ID)->row();
		echo "<tr>";		
		echo "<td><input type=\"checkbox\" value=\"$i\" name=\"cbPilih[]\" ".($res->CEK>=1?"checked":"").">&nbsp;$i</td>";		
...
$('#cabang').change(function(){
	if ($(this).val()==''){
		$('#divisi').find('option').remove().end().attr('readonly',true);
