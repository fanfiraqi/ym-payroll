...
<div class="row">	
	<div class="col-md-8">
		<div class="panel panel-default card-view"><div class="panel-heading">MASTER DATA TUNJANGAN HARI TUA</div>
		<div class="panel-body">


			<div class="row">
				<div class="col-md-8">
					<div class="form-group"><label class="col-sm-4 control-label">Kelompok Jabatan</label>
						<div class="col-sm-8"><?=form_dropdown('kelompok_jab',$kelompok_jab,'','id="kelompok_jab" class="form-control"');?></div>
					</div>
				</div>
			</div>
			<div class="row">
				<div class="col-md-8">
					<div class="form-group"><label class="col-sm-4 control-label">Pendidikan</label>
						<div class="col-sm-8"><?=form_dropdown('pendidikan',$pendidikan,'','id="pendidikan" class="form-control"');?></div>
					</div>
				</div>
			</div>
...
<script>
    $(document).ready(function() {
        $('#dataTables').dataTable({
			...
			"sAjaxSource": "<?php echo base_url('master_payroll/json_data_tht');?>"
		});
...
function changeStat(idx,  str, sts){	//nik, nama
			var pilih=confirm('Apakah data THT '+str+ ' akan diubah status ?');
			if (pilih==true) {
					$.ajax({
					type	: "POST",
					url		: "<?php echo base_url('master_payroll/changeStat');?>",
					data	: "idx="+idx+"&nmtable=mst_tht"+"&field=id"+"&status="+sts,
					timeout	: 3000,  
...
function editThis(obj){
		...
		$.ajax({
			...
			data: "id="+id+"&tabel=mst_tht"+"&field=id",
			dataType: 'json',
			success: function(msg) {
				...
				if(msg.status =='success'){
					...
					$('#kelompok_jab').val(msg.data.kelompok_jab);
					$('#pendidikan').val(msg.data.pendidikan);
	...
	$('#btsimpan').click(function(){		
		...
		$.ajax({
			type: 'POST',
			url: "<?php echo base_url('master_payroll/saveData_tht');?>",
			data: form_data,				
			dataType: 'json',
			success: function(msg) {
