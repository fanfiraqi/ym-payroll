...
<div class="row">	
	<div class="col-md-8">
		<div class="panel panel-default card-view"><div class="panel-heading">MASTER DATA TUNJANGAN MASA KERJA</div>
		<div class="panel-body">
			...
			<div class="row">
				<div class="col-md-8">
					<div class="form-group"><label  class="col-sm-4 control-label">Tahun ke </label>
						<div class="col-sm-8"><?=form_input(array('name'=>'tahun_ke','id'=>'tahun_ke','class'=>'form-control', 'value'=>0));?>	</div>
					</div>
				</div>
			</div>
			<div class="row">
...
<script>
    $(document).ready(function() {
        $('#dataTables').dataTable({
			...
			"sAjaxSource": "<?php echo base_url('master_payroll/json_data_masakerja');?>"
		});
...
function changeStat(idx,  str, sts){	//nik, nama
			var pilih=confirm('Apakah data Tunjangan Masa Kerja '+str+ ' akan diubah status ?');
			if (pilih==true) {
					$.ajax({
					type	: "POST",
					url		: "<?php echo base_url('master_payroll/changeStat');?>",
					data	: "idx="+idx+"&nmtable=mst_tunj_masa_kerja"+"&field=id"+"&status="+sts,
					timeout	: 3000,  
...
function editThis(obj){
		...
		$.ajax({
			...
			data: "id="+id+"&tabel=mst_tunj_masa_kerja"+"&field=id",
			dataType: 'json',
			success: function(msg) {
				...
				if(msg.status =='success'){
					...
					$('#tahun_ke').val(msg.data.tahun_ke);
	...
	$('#btsimpan').click(function(){		
		...
		$.ajax({
			type: 'POST',
			url: "<?php echo base_url('master_payroll/saveData_masakerja');?>",
			data: form_data,				
			dataType: 'json',
			success: function(msg) {
