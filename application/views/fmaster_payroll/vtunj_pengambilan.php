...
<div class="row">	
	<div class="col-md-8">
		<div class="panel panel-default card-view"><div class="panel-heading">MASTER DATA TUNJANGAN PENGAMBILAN</div>
			...
			<div class="row">
				<div class="col-md-8">
					<div class="form-group"><label  class="col-sm-4 control-label">Nominal Tunjangan</label>
						<div class="col-sm-8"><?=form_input(array('name'=>'nominal','id'=>'nominal','onkeypress'=>"return numericVal(this,event)", "onblur"=>"blurObj(this)","onclick"=>"clickObj(this)",'class'=>'form-control', 'value'=>0));?>	</div>
					</div>
				</div>
			</div>
			<div class="row">
...
<script>
    $(document).ready(function() {
        $('#dataTables').dataTable({
			...
			"sAjaxSource": "<?php echo base_url('master_payroll/json_data_pengambilan');?>"
		});
...
function changeStat(idx,  str, sts){	//nik, nama
			...
			if (pilih==true) {
					...
					data	: "idx="+idx+"&nmtable=mst_tunj_pengambilan"+"&field=id"+"&status="+sts,
					timeout	: 3000,  
...
function editThis(obj){
		...
		$.ajax({
			...
			data: "id="+id+"&tabel=mst_tunj_pengambilan"+"&field=id",
			dataType: 'json',
	...
	$('#btsimpan').click(function(){		
		...
		$.ajax({
			type: 'POST',
			url: "<?php echo base_url('master_payroll/saveData_pengambilan');?>",
			data: form_data,				
			dataType: 'json',
			success: function(msg) {
